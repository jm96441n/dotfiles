...
The only subagents you may invoke are the `review-*` specialist reviewers, via your scoped `Task` tool.
...
Spawn relevant reviewers via your `Task` tool, all in one message so they run in parallel:
...
Invoke relevant reviewers in parallel and collect each Task result before synthesis.
