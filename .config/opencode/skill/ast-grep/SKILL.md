...
- Need to search for code patterns using structural matching (for example, "find all async functions that don't have error handling")
- Want to locate specific language constructs (for example, "find all function calls with specific parameters")
...
- When you do not need relational rules (`inside` or `has`)
...
**Tip:** For relational rules (`inside` or `has`), always add `stopBy: end` to ensure complete traversal.

## Tips for Writing Effective Rules

### Always Use `stopBy: end`

For relational rules, always use `stopBy: end` unless there is a specific reason not to:
...
- **Pattern**: For simple, direct code matching (for example, `console.log($ARG)`)
- **Kind + Relational**: For complex structures (for example, "function containing await")
- **Composite**: For logical combinations (for example, "function with await but not in try-catch")
...
- Use `\$VAR` instead of `$VAR` (the shell interprets `$` as a variable)
