...
2. If `AGENTS.md` is missing, call `ask_question` with one question:
...
Call `ask_question` once with a `questions` array of **four** entries:
...
3. Check for collision. If `<slug>-plan-v1.md` already exists, call `ask_question`:
...
1. **Decide whether to refine.** Based on the user's Step 1 refinement intensity choice:
   - `Ask me between each round`: call `ask_question` with `header: "Refinement round <N+1>?"`, `options`:
...
- **User wants to stop mid-flow:** every `ask_question` call includes a `Cancel` / `Stop` option where appropriate.
