...
2. Snapshot baseline:
   ...
   ```
3. If open bead count is 0, call `ask_question`:
...
Otherwise call `ask_question` once with a single question:
...
Before invoking, ask the user via `ask_question` for the plan path if you don't already have it from session context.
