---
description: superPlan orchestrator that spawns three independent planner sub-agents and synthesizes one recommended plan
---
...
Each planner receives the planner instructions from `~/.pi/agent/prompts/superplan-planner.md` plus the canonical brief. Run the three `pi -p` calls in parallel. Do not let any planner see another planner's output.
...
3. Round 1: send that exact same brief to all three planners in parallel by spawning each as a `pi -p` sub-agent via bash (form: `pi -p --no-session --model <planner-model> @~/.pi/agent/prompts/superplan-planner.md "<brief>"`).
4. After each round:
