...
This command orchestrates a code review. pi has no sub-agent tool, so each specialized reviewer is spawned as a separate read-only pi sub-agent via bash: `pi -p --no-session --tools read,grep,find,ls "<reviewer prompt>"`. The `--tools read,grep,find,ls` flag enforces a read-only sandbox on each sub-agent (no `write`, `edit`, or `bash`). Do not mutate the repo yourself either — only use inspection commands needed to understand the current state. Never run mutating version control commands such as `jj describe`, `jj new`, `jj squash`, `jj bookmark set`, or `jj git push`, or Git equivalents such as `git commit`, `git commit --amend`, `git switch -c`, `git branch -f`, `git rebase`, or `git push`.
...
Spawn each relevant specialized reviewer as a read-only pi sub-agent via bash. Run them in parallel where possible. Use this form for each:

```bash
pi -p --no-session --tools read,grep,find,ls "<reviewer prompt>"
```

The sub-agent inspects the repo itself — do not read the code on its behalf. Choose reviewers based on content:

- `review-general` — Always spawn for universal code quality checks
- `review-go` — Spawn when reviewing Go code (.go files)
- `review-distributed` — Spawn when code involves distributed systems patterns (consensus, networking, service discovery, leader election, retries, circuit breakers, distributed state)
- `review-data` — Spawn when code involves database interactions (SQL, Gremlin, connection pools, transactions, queries)
- `review-architecture` — Spawn when changes touch service boundaries, shared code, or inter-service communication

Each reviewer prompt must include:

- The reviewer's focus area and the review criteria
...
- The resolved base and target refs so the sub-agent can inspect the same diff itself

Collect each sub-agent's printed findings and carry them into Step 7.
