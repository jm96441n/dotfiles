...
* **Relational Rules**: Define conditions based on a target node's position or relationship to other nodes (for example, `inside`, `has`, `precedes`, `follows`).
* **Composite Rules**: Combine other rules using logical operations (AND, OR, NOT) to form complex matching criteria (for example, `all`, `any`, `not`, `matches`).

## Anatomy of an ast-grep Rule Object

The ast-grep rule object is the core configuration unit defining how ast-grep identifies and filters AST nodes. It is typically written in YAML format.

### General Structure

Every field within an ast-grep Rule Object is optional, but at least one "positive" key (for example, `kind`, `pattern`) must be present.
...
The `regex` rule matches the entire text content of an AST node using a Rust regular expression. It is not a "positive" rule, meaning it matches any node whose text satisfies the regex, regardless of its structural kind.
...
* `"neighbor"` (default): Stops when immediate surrounding node does not match.
...
Captures a single unnamed node (for example, operators or punctuation).
...
* **Example**: `function $FUNC($$$ARGS) { $$$ }` matches functions with varying parameters and statements.
...
* **Syntax Matching**: Only exact metavariable syntax (for example, `$A`, `$$B`, `$$$C`) is recognized.
